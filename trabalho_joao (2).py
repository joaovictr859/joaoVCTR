...
if vazio(arquivo):
    atualizar(arquivo,dicionario_teste)
    cardapio_dados = dicionario_teste 
else:
    cardapio_dados = ler(arquivo)
    if cardapio_dados == None:
        print('Erro na transcrição do arquivo externo, mudando para modo interno...')
        salvar_interno = False	
...
while resp != '7':
    ...
    if resp == '1': #cadastrar
        ...
        if not(salvar_interno):
            atualizar(arquivo,cardapio_dados)
        ...
        print('=' * 30)
        for chave in cardapio_dados:
            print(f'| {chave} == R${cardapio_dados[chave][preco]}')
        print('=' * 30)
